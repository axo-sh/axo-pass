...
import {addVault, getVault, listVaults} from '@/client';
...
export class VaultStore {
  ...
  async addVault(name: string, key: string) {
    await addVault({vault_name: name, vault_key: key});
    await this.loadVaultKeys();
  }
...
export const VaultStoreContext = React.createContext<VaultStore | null>(null);

export const useVaultStore = (): VaultStore => {
  const store = React.useContext(VaultStoreContext);
  if (!store) {
